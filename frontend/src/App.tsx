...
const App: React.FC = () => {
    ...
    const onUpdatedClip = React.useCallback((clip: Clip) => {
        mutate(
            `http://localhost:8765/clips`,
            (prev: Clip[] = []) =>
                prev.map((c) =>
                    c.path === clip.path ? { ...clip, _t: Date.now() } : c,
                ),
            false,
        );
        mutate(
            `http://localhost:8765/clips/thumbnail?path=${encodeURIComponent(clip.path)}`,
        );
    }, []);
    ...
    useSocket(onNewClip, onUpdatedClip, onFfmpegUpdate);
                                ...
                                {ffmpegStatus === "error" && (
                                    <>
                                        ...
                                        <p className="text-sm text-muted-foreground text-center">
                                            ...
                                            ffmpeg.exe в папку bin рядом с
                                            Rexer.exe
                                        </p>
                                    </>
                                )}
                                ...
                                {(ffmpegStatus === "downloading" ||
                                    ...
                                    <>
                                        ...
                                        <p className="text-xs text-muted-foreground mt-2!">
                                            Подсказка: программа ffmpeg
                                            позволяет преобразовывать видео,
                                            получать из них изображения и многое
                                            другое...
                                        </p>
                                    </>
                                )}
